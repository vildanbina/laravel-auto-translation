...
class DeepSeekDriver implements TranslationDriver
{
...
EOL
    ...
    {
        $response = Http::baseUrl($this->config['api_url'] ?? 'https://api.deepseek.com')
            ->withHeaders([
        ...
        if (! $response->successful()) {
            $errorMessage = $response->json()['error']['message'] ?? 'Unknown DeepSeek error';
        ...
        if (empty($content) || ! json_validate($content)) {
            throw new Exception('Invalid JSON returned by DeepSeek: ' . json_last_error_msg());
        }

        $decoded = json_decode($content, true);

        if (count($decoded) !== count($texts)) {
            throw new Exception('Mismatch in number of translated texts returned by DeepSeek.');
        }
